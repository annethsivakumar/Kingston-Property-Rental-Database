...
<body>
    ...
    <div class="white-box">
        <?php
            ...
            // Insert Room Furnishings
            if (!empty($_POST["property_ID2"]) && !empty($_POST["furnishings_list"])) {
                $property_ID2 = !empty($_POST["property_ID2"]) ? $_POST["property_ID2"] : 'null';
                $furnishings_list = !empty($_POST["furnishings_list"]) ? "'" . $_POST["furnishings_list"] . "'" : 'null';
                $sql = "insert into RoomFurnishings values ($property_ID2, $furnishings_list);";
                $sql = str_replace("'null'", "null", $sql);
                if ($connection->query($sql) == TRUE) {
                    echo "<p>New room furnishings created successfully.</p>";
                } else {
            ...
            // Insert Room
            $roommate_count = !empty($_POST["roommate_count"]) ? $_POST["roommate_count"] : 'null';
            $kitchen_privileges = !empty($_POST["kitchen_privileges"]) ? "'" . $_POST["kitchen_privileges"] . "'" : 'null';
            $sql = "insert into Room values ($property_ID, $roommate_count, $kitchen_privileges);";
            $sql = str_replace("'null'", "null", $sql);
            if ($connection->query($sql) == TRUE) {
                echo "<p>New room created successfully.</p>";
            } else {
                echo "<p>Error: " . $sql . "<br>" . $connection->error . "</p>";
            }
