...
<body>
    ...
    <div class="white-box">
        ...
        <?php
            ...
            $result = $connection->query("SELECT group_code FROM RentalGroup ORDER BY group_code ASC;");
            while ($row = $result->fetch()) {
                $group_code_zeros = sprintf("%04d", $row["group_code"]);
                echo "<tr><td><a href='http://localhost/displayRentalGroupRenters.php?group_code=".$row["group_code"]."'>".$group_code_zeros."</a></td></tr>";
            }
            $connection = NULL;
        ?>
        </table>
